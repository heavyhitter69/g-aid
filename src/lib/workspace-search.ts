 ...
 * header/content grep. Disk grep lives in workspace-search-fs.ts so the
 * in-memory index search can run in the client bundle.
 */
...
export const SEARCH_SKIP_DIRS = SKIP_DIRS;
...
export function scoreHaystack(haystack: string, needles: string[]): number {
