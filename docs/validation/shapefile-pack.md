...
| Geometry | ESRI types 1 / 3 / 5 / 8 only: Point, PolyLine, Polygon, MultiPoint (flattened to Point/LineString/Polygon). |
...
Z/M/MultiPatch types, null-only or empty datasets, malformed polygon rings.
...
4. Map view, same-CRS overlap, and GeoJSON export use the existing GIS nodes.
5. Overlap is geometric coincidence, not geological causation.
